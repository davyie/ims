...
# ─────────────────────────────────────────────
# IMS Test Build Script
# Builds backend JAR + Docker image, then
# tears down any running test containers and
# brings the test stack back up.
# ─────────────────────────────────────────────
...
for arg in "$@"; do
  case $arg in
    ...
    --help|-h)
      echo "Usage: ./build-test.sh [options]"
      echo ""
      echo "Options:"
      echo "  --with-tests   Run Maven tests during build (skipped by default)"
      echo "  --no-cache     Build Docker images with --no-cache"
      echo "  --clean        Run 'docker compose down -v' before starting (wipes test DB data)"
      echo "  --help         Show this help"
      exit 0
      ;;
    *) die "Unknown option: $arg  (run ./build-test.sh --help for usage)" ;;
  esac
done
...
log "Starting IMS test-stack build…"
echo ""
...
# ── step 3: Compose up ───────────────────────────────────────
log "Step 3/3 — Starting test containers with Docker Compose"

if $DOWN_VOLUMES; then
  warn "--clean specified: stopping test containers and wiping volumes"
...
ok "All test containers started"
echo ""
